		...
		this.each(function(){
			...
			window.onload = function() {
				...
				if((typeof window.orientation != 'undefined') || (navigator.userAgent.indexOf('Windows Phone') != -1)){
					$('.carousel-slide').addClass('mobile');
				} // Hide controls for mobile

				$(container).fadeIn(1000);
				setTimeout(function(){
					$('.carousel-slide li:first').addClass('current');
				}, 200);

				$('.thumbnails li:first').addClass('selected');
				$('.carousel-slide').height($('.carousel-slide li:first').outerHeight() - 1);
				...
				function changeSlide(number) {
					if(options.changeHeight){
						$('.carousel-slide').height($('.carousel-slide li').eq(number).outerHeight() - 1);
					}
					$('.carousel-slide li').removeClass('current');
					$('.carousel-slide li').eq(number).addClass('current');
				...
				$(window).on("resize", function() {
					...
					timer = setTimeout(function() {
						$('.carousel-slide').height($('.carousel-slide li').eq(selectedNumber).outerHeight() - 1);
					}, 50);
				});
			}
		});
